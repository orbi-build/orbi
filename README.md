...
- `model_wait_dead issue=... role=... idle_seconds=... threshold=... action=kill_pi session=... run_id=... upstream_alive=true|false reason=hung_model_request`——`model_wait` 静默超过 `PI_MODEL_WAIT_DEAD_SECONDS`（默认 600 秒）时输出一次 WARNING 并杀掉 Pi 会话（Issue #218）：`upstream_alive` 是证据不是否决（进程活着 ≠ 在回话），`run_id` 是低频行字段（见下）；
