...
| Key | Type | Default | Allowed values | Meaning |
|---|---|---|---|---|
| [`steering_enabled`](/workflow#steering-an-active-delivery) | boolean | `true` | `true \| false` | Enables trusted Issue correction polling. |
| [`steering_poll_seconds`](/workflow#steering-an-active-delivery) | number | `60.0` | finite positive number | Interval between steering polls. |
| [`steering_max_rounds`](/workflow#steering-an-active-delivery) | integer | `3` | non-negative integer | Maximum steering restarts per delivery. |
