...
1. **Fast-forwards the code first** (`ExecStartPre`, outside the Python
   ...
   console script (`muyan-pilot --version` succeeds iff the package
   ...
   cannot even `import muyan_pilot` — the #248 scene, where the
...
and `muyan-pilot setup` verifies or reinstalls it. The checkout root
carries no `muyan_pilot.py`: a flat file named like the package would
shadow the installed package for every process with the checkout root
on sys.path. The direct-execution entry `python3 -m muyan_pilot.cli`
