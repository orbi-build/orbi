...
2. **Runs one tick**: resume an opened PR (review/fix/merge) or claim one
   ...
   executes matches the fetched `origin/<base_branch>` — see
...
- **Editable install** (the official deployment): the gate resolves the
  ...
  checkout's `HEAD` to equal the fetched `refs/remotes/origin/<base>`
  ref. Worktrees share the deployment home's refs, so the incident
  scene (an install bound to a stale worktree) is rejected here.
- **Non-editable install** (uv tool / PyPI copy): the installed
  distribution version (`importlib.metadata`) must not be older than
  the latest release tag reachable from the fetched base ref (resolved
  in the deploy home). Equality or newer passes.
- Everything is a LOCAL git read (`rev-parse` / `describe`): the
  freshness of the remote-tracking ref is supplied by the
  `ExecStartPre` fetch, so a fresh checkout costs zero network
...
- A stale or unverifiable source logs the structured
  `runner_source_stale install=... source=... head=... origin_main=...
  allowed=false fix=...` line and fails the start: no slot, no claim,
  no label change. The `fix` is the exact editable force-reinstall
  against the deploy home. `allow_stale_runner: true` (getting-started)
  downgrades the SAME line to a warning for offline/restricted-network
  machines — it never skips the check.
...
1. The **shell-layer `ExecStartPre` preflight** (fetch + `merge
   --ff-only` + the CLI probe self-heal) is the OUTERMOST defense: it is
   executed by systemd without importing any runner code, so it never
   depends on the runner version. Keep the installed units current (the
   `unit_drift` self-heal of `install-units` and the pre-start check)
   so this preflight always exists.
...
- `pi_retry_429` — one WARNING per provider rate-limit retry (Issue
  ...
  new Issue is claimed while this run waits). After 5 consecutive 429
  exits the EXISTING failure path runs — the same error type a
