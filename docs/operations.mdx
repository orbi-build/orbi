...
- `pi_idle_wait` — the grace line of the idle-stall recovery (Issue
  ...
  idle windows;
- `command_deadline_extension_missing` — the pure bypass of the
  command-deadline contract (Issue #1093): the engine ships a Pi
  extension (`pi_extensions/command_deadline.ts`) that bounds every
  bash tool call at `ORBI_COMMAND_DEADLINE_SECONDS` (the Runner sets it
  to `PI_IDLE_WAIT_MAX_SECONDS`, the single absolute-timeout constant)
  and rewrites an unwrapped command to
  `timeout <deadline> bash -c '<original>'`, so every command carries a
  declared deadline on its process command line and the
  `pi_idle_wait` grace applies to it; a command past the deadline exits
  124 with its own output instead of the session being SIGTERMed by the
  idle escalation. The WARNING logs when the shipped file cannot be
  found (a broken install) and the session continues with the
  pre-#1093 behavior; a missing extension never fails a delivery;
...
# (the manual command behind the auto_next_milestone=false
# confirmation flow; only the active_milestone line changes)
...
`orbi milestone set <title>` (Issue #895) is the human command in the
milestone lifecycle: when `auto_next_milestone = false` leaves the
advance pending (the confirmation Issue names this command), or
whenever a version switch should be a deliberate operator step, it
moves `active_milestone` in the local `orbi.toml` to the given GitHub
Milestone title — no hand-editing the file. The title must exist as
exactly ONE Milestone on the single configured source repo (GitHub
Milestone titles are not unique; a duplicate exact title is a hard
error). Only the `active_milestone` line is rewritten: comments, blank
...
The command itself never writes to GitHub — the Milestone list read is
its only network call. The `ORBI_ACTIVE_MILESTONE` Actions variable is
...
leaves the config untouched — a missing or duplicated title, a GitHub
API or permission failure, a config without the `active_milestone`
line (the field is never created implicitly), or an unwritable file
all fail fast with the actual reason:
...
- **CLI:** the bare `orbi` tick and the `add`, `status`, `doctor`, `setup`,
  `install-units`, `session`, and `milestone set` subcommands, including
  `--config` and the `ORBI_CONFIG` environment variable.
