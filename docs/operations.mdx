...
# (the host-side move; the auto_next_milestone=false confirmation
# Issue carries the in-ticket `/milestone <title>` command; only the
# active_milestone line changes)
...
- The three steps are idempotent: create the Milestone (open) when it is
  ...
  detection, else `pyproject.toml`; a missing template falls back to the
  built-in minimal field set and says so in the ticket), then land
