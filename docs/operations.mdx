...
- `run_failed` — the full scene plus the reason
  ...
  frozen `model_wait` past the configured `model_wait_dead_seconds`
  (default 1800 s, Issue #228) declares the model request hung, or
  `idle_recovery_stale_...s` when the session stayed idle for
  `PI_IDLE_RECOVERY_CYCLES` (default 3) consecutive idle windows and
  the Runner kills the Pi session itself).
