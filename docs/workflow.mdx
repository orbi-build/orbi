...
- **Release task**: an `ai-ready` Issue additionally marked with the
  ...
  3. Enforce the pre-release gates: no leftover open `ai-in-progress` /
     ...
     with its own reason, never reported as a CI failure), and no open
     PRs targeting the base branch. A gate that cannot be checked
     because of a real `gh` failure is a failed gate.
