...
- **Epic**: a coordination Issue that groups a set of related tasks (for
  ...
  excludes it too. The Epic is complete exactly when every declared child is closed (or its PR
  is merged) and there is no open native `blockedBy` dependency. The Runner
  reconciles open Epics on every tick and closes only when those GitHub facts
  are proven, leaving an idempotent audit comment. An absent or malformed
  child scope or dependency field fails closed. Any prose describing extra
  acceptance conditions is an Epic definition error: make that condition a
  final child Issue instead (for example, a browser acceptance ticket).
