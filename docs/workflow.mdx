...
another pool. The run comment records `repo_config: <sha>`; when the sha
differs from the previous run, the change is annotated
(`repo-config-changed`) with the effective policy diff, so a change is
