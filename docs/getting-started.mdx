...
| Pi starts but the model never answers | The session JSONL stalls in `model_wait`; after the dead-silence window the journal shows the `model_wait_dead` line and `run_failed ... reason=model_wait_dead_stale_...` | The model request is hung — the model server/proxy is down, its connection dropped, or the request is stuck while the process is still alive (`upstream_alive=true` on the line is evidence, not a veto) — check the endpoint with one real `pi --print` call, restart it, and re-dispatch (the Issue is `ai-blocked` with the scene) |
