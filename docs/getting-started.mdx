...
The installer requires `git`, `gh`, and a systemd user session; uv provisions
...
The released CLI installs from PyPI (Issue #163) — no source checkout
required:

```bash
python3 -m pip install orbi
orbi --version
orbi --help
```

uv users install the same published package non-editable:

```bash
uv tool install orbi
```

**PyPI package vs editable deployment**: the PyPI install is the
*versioned release CLI* — a frozen copy of one release in site-packages,
upgraded by reinstalling (`python3 -m pip install --upgrade orbi` /
`uv tool install --reinstall orbi`). The editable checkout install below
stays the official DEPLOYMENT: it imports the `orbi` package from the
deployment checkout, so the `ExecStartPre` sync delivers new engine code
to the next CLI process with no reinstall. Running a Runner still needs
that deployment layout (config, deploy home, systemd user units); verify
a machine's readiness with the read-only prerequisite gate `orbi check`
([Operations](/operations) carries the command reference).

## Prerequisites

You install these yourself — `orbi setup` only **checks** them
(it verifies `git`, `gh`, `uv` and the systemd user session on the
machine; uv provisions the required Python interpreter). It never installs
...
| `mergeable_wait_seconds` | no | `120` | Merge gate mergeability wait (Issue #409): maximum seconds to poll a PR whose `mergeable` is `UNKNOWN` after CI settles. Must be a finite positive number; anything else fails the start fast |
