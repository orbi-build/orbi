...
| `engine_source_track` | no | `main` (fresh installs via `install.sh`: `release`) | Engine source update channel (Issue #535, host/deploy-only — never a repository's `.github/orbi.toml`): what the deploy home checkout follows at every service start. absent/`main`: fast-forward `origin/main` (the pre-#535 dogfood behavior). `branch:<name>`: fast-forward `origin/<name>`. `release`: the newest official semver tag (pre-releases excluded), detached. `tag:<name>`/`sha:<40-hex>`: exact lock, detached — rollback is editing the value back. A dirty checkout, a missing tag/SHA or any unverifiable state fails the start with a structured line (see [Operations](/operations)) |
