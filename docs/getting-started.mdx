...
| `active_milestone` | no | unset | Claim scope for the fresh-claim scans (Issue #139): the active GitHub Milestone title (e.g. `v0.2.0`). Set it to restrict the p0/bug/plain ready scans to that Milestone (the `milestone:"<title>"` qualifier is part of the gh search query — an Issue of another or no Milestone never enters the queue); `ai-ready` stays the execution switch, the pickup order is unchanged, P0 does not cross milestones, and resume states (opened PRs, in-flight restarts) are never gated by it. Unset = every `ai-ready` Issue is claimable (pre-#139 behavior). Auto-advance (Issue #274): when a tick's ready scan ends with `no_ready_issue` and this field is set, the Runner checks the Milestone by EXACT title — still `open` → nothing happens; `closed` → the value is advanced to the smallest open `v<major>.<minor>.<patch>` Milestone that is semver-strictly-higher and written back to this file (only that line); missing or duplicate title → fail fast; no higher candidate → `active_milestone_advance_none`, value kept |
