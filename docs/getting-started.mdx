...
Python 3.14 when needed. For manual installations, `orbi setup` creates `orbi.toml`
when absent — from the adjacent `.orbi.example.toml` when one exists, otherwise
from the example shipped inside the CLI package — without overwriting it.
...
| [Pi](https://github.com/earendil-works/pi) CLI on Node ≥ 22.19.0 | The development agent; one full session per task — install with `npm install -g --ignore-scripts @earendil-works/pi-coding-agent` | `pi --version` |
