...
| `model_wait_dead_seconds` | no | `1800` | Hung-model-request threshold (Issue #228): seconds of `model_wait` silence (the newest complete session event is a tool result and no new event arrives) before the Runner kills the Pi session and fails fast. It measures silence between **complete** Pi session events — Pi does not stream token-level progress into the JSONL, so it is not a token-level model timeout and a slow generation that keeps appending events never trips it. Default 1800 (30 min) lets a slow local model (e.g. Qwen 27B at ~17 tokens/s behind a llama-server with a 1200 s request timeout) survive a 10-minute complete message. Must be a finite positive number; anything else fails the start fast |
| `model_wait_probe_url` | no | unset | Swallowed-model-request probe (Issue #233): the model's `/slots` endpoint (e.g. `http://127.0.0.1:18082/slots`, the agent-facing port — the local-llm-kv-cache proxy passes it through from llama.cpp). While Pi is frozen in `model_wait` the Runner probes it; when **every** slot reports idle (`is_processing=false`) for the sustained `model_wait_probe_seconds` grace the request was SWALLOWED (the model process is alive and the connection ESTABLISHED, but nothing is generating — the #231 scene) and the Pi session is killed fast, well before `model_wait_dead_seconds`. Unset = the probe is disabled (the run is bounded by `model_wait_dead_seconds` only, the pre-#233 behavior). Must be a non-empty `http://`/`https://` URL; anything else fails the start fast. The probe is a pure bypass: a probe failure is inconclusive and never fails the delivery |
| `model_wait_probe_seconds` | no | `60` | The sustained-idle grace for the swallowed-model-request probe (Issue #233): how long the "every slot idle" state must hold (while in `model_wait`) before the request is declared swallowed and Pi is killed. Short (60 s) compared to the dead-request threshold (30 min) so a swallow is recovered in ~1 minute, not ~30; long enough that a request still being scheduled into the slot is never misread as a swallow. Must be a finite positive number; anything else fails the start fast |
...
| Pi starts but the model never answers | The session JSONL stalls in `model_wait`; after the dead-silence window (`model_wait_dead_seconds`, default 1800 s — silence between complete session events, not token-level progress) the journal shows the `model_wait_dead` line and `run_failed ... reason=model_wait_dead_stale_...` | The model request is hung — the model server/proxy is down, its connection dropped, or the request is stuck while the process is still alive (`upstream_alive=true` on the line is evidence, not a veto) — check the endpoint with one real `pi --print` call, restart it, and re-dispatch (the Issue is `ai-blocked` with the scene); a slow local model that is still generating survives the window (raise `model_wait_dead_seconds` only if your model legitimately needs more than 30 minutes of complete-event silence) |
