...
| `allow_stale_runner` | no | `false` | Offline escape hatch for the runner source freshness gate (Issue #525): before any slot or claim the Runner proves the code it executes matches the fetched `origin/<base_branch>` (the import checkout's `HEAD` for an editable install; the installed version vs the latest release tag for a non-editable one — local git reads only). A stale or unverifiable source logs the structured `runner_source_stale` line (facts + fix) and fails the start. `true` downgrades the SAME line to a warning so an offline/restricted-network machine keeps running — it never skips the check. Invalid values fail fast |
