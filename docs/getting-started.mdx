---
description: "What do I need to self-host an AI coding agent that merges and releases?"
---
# Getting started

You need a GitHub repository, the Orbi CLI and scheduler, an authenticated model provider, and a local user session to run the delivery loop.
...
New users only need to run the installer; it installs the CLI, creates the
config, and starts the interactive setup:

```bash
curl -LsSf https://raw.githubusercontent.com/orbi-build/orbi/main/install.sh | bash
```

For a reviewable alternative to `curl | bash`, download and inspect first:
...
The installer requires `git`, `gh`, and a scheduler user session
(systemd on Linux, launchd on macOS); uv provisions
Python 3.14 when needed. It also installs the
[Pi](https://github.com/earendil-works/pi) CLI for you (via
`npm install -g --ignore-scripts @earendil-works/pi-coding-agent`) —
Node ≥ 22.19, which provides npm, is the one prerequisite it does not
install: install Node yourself from the
[official download page](https://nodejs.org/en/download) and the
installer verifies the version. For manual installations, `orbi setup`
creates `orbi.toml` when absent — from the adjacent `.orbi.example.toml`
when one exists, otherwise from the example shipped inside the CLI
package — without overwriting it.

## PyPI installation

The CLI is published on PyPI as
[`orbi-cli`](https://pypi.org/project/orbi-cli/) — sdist + wheel,
requiring Python ≥ 3.14. The distribution name is
`orbi-cli`; the installed command stays `orbi`.

`uv tool install orbi-cli` installs it into an isolated tool
environment (uv supplies a compatible Python 3.14 interpreter when
the system one is older); inside an activated Python ≥ 3.14
environment, `pip install orbi-cli` is the plain pip path:
...
The ≥ 3.14 floor applies to the interpreter that runs the install:
an explicit `--python 3.13` on the `uv tool install` call fails with
the uv resolver message
`orbi-cli==<version> depends on Python>=3.14`; `pip install orbi-cli`
inside an activated older environment reports
`No matching distribution found for orbi-cli`. The repair in both
cases is the same: select a compatible interpreter — point
`uv tool install` at `--python 3.14` (uv provisions it) or run pip
inside an environment built on Python ≥ 3.14.

Uninstall the tool install with `uv tool uninstall orbi-cli` (the
full removal story lives in
[Operations](/operations)); these installs give you the `orbi`
command only. To run Orbi itself, use the one-line installer above or
the editable checkout install in step 2 — Orbi's deployment drives an
editable install so the CLI follows the synced checkout.

## Prerequisites

You install these yourself — `orbi setup` only **checks** them
(it verifies `git`, `gh`, `uv` and the scheduler user session on the
machine — systemd on Linux, launchd on macOS; uv provisions the
required Python interpreter). It never installs
system packages, never installs
`gh`, and never runs `gh auth login` (you log in first, setup only
verifies the login). `orbi check` is the read-only prerequisite gate over
the same list plus the Python version, the `pi` CLI, the config, per-repo
access, the git transport and the model provider — it fails fast with the
reason, the repair action and the official docs link.

| Requirement | What it is used for | Check |
|---|---|---|
| Python 3.14 | The Runner and the test contract (CI pins the same minor version) | provisioned by `uv` |
...
| [Pi](https://github.com/earendil-works/pi) CLI on Node ≥ 22.19.0 | The development agent; one full session per task — installed by the one-line installer (via npm); to install it manually: `npm install -g --ignore-scripts @earendil-works/pi-coding-agent` | `pi --version` |
| Git | Worktrees, branches, base freshness | `git --version` |
| [GitHub CLI (`gh`) ≥ 2.94](https://github.com/cli/cli/blob/trunk/docs/install_linux.md) | Issues, labels, PRs, merge | `gh --version`, then `gh auth status` |
| systemd user session (Linux) | The user timer that triggers one tick every 5 minutes | `systemctl --user status` |
| launchd (macOS) | The user agent that triggers one tick every 5 minutes — not yet verified on real hardware, reports welcome | `launchctl print gui/$(id -u)` |
...
Install `gh` from the official GitHub CLI repository — the minimum tested
version is **2.94.0**: the Runner's queue scans use `gh api --paginate --slurp`
(added in gh 2.48.0) and the native `blockedBy` dependency field (added in
gh 2.94.0). A too-old package passes `orbi setup` and then fails every tick
with `unknown flag: --slurp` / `Unknown JSON field: "blockedBy"` and no
Issue is ever claimed — Ubuntu 24.04's distribution package (2.45.0) is
such a case; Fedora (2.97.0) and Arch (2.100.0) currently satisfy the
minimum natively. On Ubuntu/Debian, install or repair from the official
apt repository (the [official Linux install
instructions](https://github.com/cli/cli/blob/trunk/docs/install_linux.md)):
...
Pi's npm package declares a **Node ≥ 22.19.0** floor in its `engines`, but
npm does not enforce it: on an older Node the `npm install` above still
reports success, and then every `pi` invocation crashes with a bundle-level
`SyntaxError` — the hardest-to-diagnose failure on the install path. Debian
trixie and Ubuntu 24.04 ship distro-default Node versions older than that
floor. The repair is to install Node ≥ 22.19 and reinstall Pi with the
command above; `orbi check` executes `pi --version` (a PATH hit alone is
not enough) and names this floor in its `fix=` when the probe fails.
...
the clone directory, pass `--python` a COMPATIBLE interpreter
(Issue #861): the system `python3` when it is 3.14 or newer (Fedora 43
and current Arch ship it), otherwise `3.14` so uv provisions or
selects one — Ubuntu 24.04 ships Python 3.12, and an exact-path pin to
that system interpreter exits 1 with a dependency resolution failure,
while the bare `python3` and `3.14` forms let uv select or provision a
satisfying interpreter (a managed CPython 3.14 on such a host):

```bash
# system Python >= 3.14 (Fedora 43, current Arch):
uv tool install --force --reinstall --editable --python python3 .
# older system Python (e.g. Ubuntu 24.04 ships 3.12) — uv provisions 3.14:
...
lands in `~/.local/bin` (on the PATH of the scheduler unit).
...
for that class of breakage the service's FIRST `ExecStartPre` step
self-heals OUTSIDE Python: it probes `orbi --version` and, on
probe failure, runs the same editable force-reinstall — before the
second step (`orbi sync-engine-source`, Issue #535), which needs that
working CLI (see
...
`orbi setup` creates `orbi.toml` automatically when it is absent.
...
| `review_pi_provider` | no | unset | Provider override for review sessions; unset falls back to `pi_provider` |
| `review_pi_model` | no | unset | Model override for review sessions; unset falls back to `pi_model` |
| `review_pi_thinking` | no | unset | Thinking-level override for review sessions; unset falls back to `pi_thinking` |
| `pi_extensions` | no | `[]` | Repeatable extension tables: `source` is a pinned `npm:`/git source or an existing config-relative local path; `enabled` defaults to true and `env` is injected only into that Pi child. Every run passes `--no-extensions`, so global Pi extensions are never discovered. |
...
prompt_review = "prompts/prompt_review.md"
base_branch = "main"
engine_source_track = "release"  # Issue #535: engine source channel (see Operations)
max_concurrency = 1
skills = []
context_files = []
```
...
degree: the median across this repository's own deliveries is
...
| The PR is behind the latest base | The Issue is `ai-fix-needed` with a review comment naming the concrete gate failure: a behind-base head carries the `origin/<base>` SHA, a merge conflict carries the actual `mergeable` value (Issue #879) | Automatic: the next review session merges the latest `origin/<base>` into the branch in-session, resolves conflicts, re-runs the full suite |
...
through `max_concurrency`: `@1` through `@N` for `N` (up to
`MAX_RUNNER_INSTANCES = 5`; each instance
...
(The timers). At capacity `N`, N instances may run concurrently; the
