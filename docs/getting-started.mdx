...
```bash
curl -LsSf https://orbi.build/install.sh | bash
```

For a reviewable alternative to `curl | bash`, download and inspect first:

```bash
curl -LsSf https://orbi.build/install.sh -o install.sh
less install.sh
bash install.sh
```

The installer requires `git`, `gh`, and a systemd user session; Python is
managed by uv. For manual installations, `orbi setup` creates `orbi.toml`
