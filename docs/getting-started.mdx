...
| `auto_next_milestone` | no | `true` | Controls idle advancement after the active milestone closes and its queue is empty. `false` leaves `active_milestone` unchanged, logs a pending warning, and creates one `ai-ready` + `p0` confirmation Issue; invalid values fail fast |
