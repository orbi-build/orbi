...
from the adjacent `.orbi.example.toml` when absent, without overwriting it.
...
| `base_branch` | no | `main` | Delivery target base branch; every task worktree is created from the frozen `origin/<base_branch>` SHA |
| `source_branch` | no | `main` | Orbi engine source branch for freshness checks; independent from the delivery `base_branch` (for example, a website target may use `beta`) |
...
| `allow_stale_runner` | no | `false` | Offline escape hatch for the runner source freshness gate (Issue #525): before any slot or claim the Runner proves the code it executes matches the fetched `origin/<source_branch>` (the import checkout's `HEAD` for an editable install; the installed version vs the latest release tag for a non-editable one — local git reads only). A stale or unverifiable source logs the structured `runner_source_stale` line (facts + fix) and fails the start. `true` downgrades the SAME line to a warning so an offline/restricted-network machine keeps running — it never skips the check. Invalid values fail fast |
