...
Configuration is kept in `orbi.toml` (relative paths resolve from the
config file directory). `ORBI_CONFIG` selects another file, and `orbi check`
validates it. The complete reference is [Configuration reference](/configuration).

A minimal first-run configuration is:

```toml
source_repos = ["OWNER/PILOT-REPO"]
repo_dir = "."
workspace_root = ".."
...
`orbi setup` creates `orbi.toml` from the shipped example when it is absent.
For external single-repo deployments, set `deploy_home` to the Orbi checkout
and follow the [configuration reference](/configuration).
