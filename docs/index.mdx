...
Orbi is a local AI development worker. You put work into GitHub Issues; it automatically claims an `ai-ready` Issue, develops it in an isolated worktree, runs the real test suite, opens a PR, then completes an independent review (with in-session fixes) and merges a clean PR — with the whole delivery recorded in GitHub Issues, comments and the PR itself. The reviewing session can run on a **different model** than the implementing session ([learn how](/workflow); the different-model review subsection is being added separately). This MVP keeps state only in GitHub rather than a database or queue, runs one Issue per tick rather than a daemon, and does not discover repositories or push to protected branches itself. See [/security](/security) for the full boundary list.
...
<CardGroup>
  ...
  <Card title="Independent review, optionally on a different model" href="/workflow">
  ...
  <Card title="You can steer a running delivery" href="/operations#event-reference">
    A trusted correction on the Issue restarts the run with it. This lets you correct a delivery while it is running.
  </Card>
  ...
  <Card title="Self-hosted core, hosted control plane" href="/getting-started">
    The runner is open source and runs on your machine. [Orbi Managed Cloud](https://orbi.build/cloud/) runs the same ledger without you operating a runner.
  </Card>
</CardGroup>
...
- [Ways to run Orbi](/getting-started): self-host the runner or use Managed Cloud.
