...
| Key | Type | Default | Allowed values | Meaning |
|---|---|---|---|---|
| [`steering_enabled`](/zh/workflow#运行中的交付修正) | 布尔值 | `true` | `true \| false` | 启用受信任的 Issue 修正轮询。 |
| [`steering_poll_seconds`](/zh/workflow#运行中的交付修正) | 数字 | `60.0` | 有限正数 | Steering 轮询间隔。 |
| [`steering_max_rounds`](/zh/workflow#运行中的交付修正) | 整数 | `3` | 非负整数 | 每个交付最多重启的 Steering 次数。 |
