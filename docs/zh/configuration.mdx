...
| `auto_next_milestone` | 布尔值 | `true` | `true \| false` | 当前 Milestone 关闭且队列为空时是否自动推进。`false` 保持 `active_milestone` 不变，记录待确认告警并创建一个不带 `ai-ready` 或 `p0`、不会被领取的确认 Issue（票内自带 `/milestone <版本>` 命令，见 Issue #1290）；人工推进后旧通知会自动关闭。非法值立即失败 |
