...
| `active_milestone` | 字符串或空 | 未设置 | 任意非空字符串 | 新领取扫描的领取范围（Issue #139）：当前活跃的 GitHub Milestone 标题（如 `v0.2.0`）。领取前 Runner 会按 exact title 在源仓库解析：标题不存在时结构化失败并停止；标题已关闭时只记录 INFO（含该 Milestone 下的 `ai-ready` 数量），不替维护者判断版本是否正确。设置后 p0/bug/普通三个 ready 扫描只领取该 Milestone 的 Issue（`milestone:"<title>"` 限定词直接进 gh 搜索查询——其他 Milestone 或无 Milestone 的 Issue 永远进不了队列）；`ai-ready` 仍是执行开关，领取顺序（p0 → bug → 普通）不变，P0 不跨 Milestone，恢复态（已开 PR、在途重启）不受 Milestone 限制。唯一例外是外部接管扫描（Issue #842 决策 D1）：第五条、排最后的 ready 扫描不带 Milestone 限定，带正文标记 `<!-- orbi:external-pr:N -->` 的外部 triage 票始终可被认领——外部贡献不属于任何版本的范围，绝不能阻塞发版门禁。未设置 = 所有 `ai-ready` Issue 都可领取（#139 之前的行为）。用 `orbi milestone set <title>` 推进（见[运维](/zh/operations)；Runner 下一 tick 发布 `ORBI_ACTIVE_MILESTONE` variable） |
| `auto_next_milestone` | 布尔值 | `true` | `true \| false` | 当前 Milestone 关闭且队列为空时是否自动推进。`false` 保持 `active_milestone` 不变，记录待确认告警并创建一个不带 `ai-ready` 或 `p0`、不会被领取的确认 Issue（票内自带 `/milestone <版本>` 命令，见 Issue #1290）；人工推进后旧通知会自动关闭。**可写入仓库的 `.github/orbi.toml`**（Issue #1342）：仓库声明的值胜过本键，省略时回退到宿主值，因此托管租户也能阻止引擎改写 `active_milestone`。非法值立即失败 |
| `version_file` | 字符串或空 | 未设置 | `pyproject.toml`、`package.json`、`pom.xml`、`build.gradle`、`build.gradle.kts`、`gradle.properties`、`Cargo.toml`、`composer.json`、`pubspec.yaml` 或 `none` | 票内 `/milestone <版本>` 命令（Issue #1290，仅宿主/部署侧——仓库的 `.github/orbi.toml` 不可携带）开的 release 票里写死的版本文件。未设置 = Runner 从 base 分支的文件树探测；探测找不到受支持文件时 `/milestone` 的 release 票步骤直接失败并在票上留一条指出修复方式的回执，不再猜测 `pyproject.toml`（Issue #1307）；取值不在列表内立即失败 |
...
| `issue_comments_limit` | 整数 | `200` | 正整数 | 受信任评论注入上限（Issue #745，仅宿主/部署侧——仓库的 `.github/orbi.toml` 不可携带）：`{{ISSUE_COMMENTS}}` 占位符注入多少条受信任评论（OWNER/MAINTAINER/MEMBER/COLLABORATOR 或 runner 自己的 App——与恢复现场同一套 `authorAssociation` 信任判据），按时间正序、保留最新。超出上限时丢弃更早的，并在注入内容里标明省略条数——绝不静默截断。模板不含该占位符时不发起任何评论读取（#745 之前的行为）。必须是正整数，否则启动 fail fast |
...
| `clarify_thin_tickets` | 布尔值 | `false` | `true \| false` | 薄 ticket 闸门（Issue #1088）。可写在仓库的 `.github/orbi.toml`；仓库声明值优先，未声明的键沿用本值。`true` 时在领取开工之前用一次无工具 Pi session 判定 ticket 正文的三项检查：可观察结果、验收条件、单一结果。判定不通过时只发 ONE 条列出全部缺失项的评论，把 `ai-ready` 换成 `ai-needs-detail`，并中止本次尝试——不建 worktree、不建分支、不起 Pi session、不开 PR；票像 `ai-blocked` 一样等人补齐。所有错误路径（模型报错、超时、答案不可用、评论或标签写入失败）一律 fail OPEN：记 `clarify_check_skipped`，交付照常继续。默认关闭，未启用的仓库行为不变。参见[薄 ticket 闸门](/zh/workflow#薄-ticket-闸门)。 |

## Steering

Steering 让 Runner 轮询 Issue 中的受信任修正，并带着修正重启正在进行的交付。`steering_max_rounds` 限制重启次数，避免交付被无限重定向。轮询失败属于纯旁路：会记录日志，但绝不会让交付失败。这三个键都可以写在仓库的 `.github/orbi.toml` 中；仓库声明的值优先，省略时回落到 host 值。`steering_poll_seconds` 必须是有限正数，不额外设置下限。

详见 Operations 中的 [`delivery_steered`、`steering_limit_reached`、`steering_poll_failed`、`steering_limit_notice_failed`](/zh/operations) 行。

| Key | Type | Default | Allowed values | Meaning |
|---|---|---|---|---|
| `steering_enabled` | 布尔值 | `true` | `true \| false` | 启用受信任的 Issue 修正轮询。参见[运行中的交付修正](/zh/workflow#运行中的交付修正)。 |
| `steering_poll_seconds` | 数字 | `60.0` | 有限正数 | Steering 轮询间隔。可写在 `.github/orbi.toml` 中；不额外设置下限。参见[运行中的交付修正](/zh/workflow#运行中的交付修正)。 |
| `steering_max_rounds` | 整数 | `3` | 非负整数 | 每个交付最多重启的 Steering 次数。参见[运行中的交付修正](/zh/workflow#运行中的交付修正)。 |
