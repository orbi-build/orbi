...
2. **运行一个 tick**：恢复一个已打开的 PR（review/fix/merge），或领取
   ...
   的代码与已 fetch 的 `origin/<base_branch>` 一致——见下文
...
- **editable 安装**（官方部署形态）：门禁解析**运行中进程**的 import
  ...
  checkout 的 `HEAD` 等于已 fetch 的 `refs/remotes/origin/<base>`
  ref。worktree 与部署 home 共享 refs，所以事故场景（install 绑到旧
  worktree）在这里被拒绝。
- **非 editable 安装**（uv tool / PyPI 拷贝）：安装的发行版本
  （`importlib.metadata`）不得旧于已 fetch base ref 上可达的最新
  release tag（在 deploy home 解析）。相等或更新即通过。
- 全部是**本地 git 读**（`rev-parse` / `describe`）：remote-tracking
  ref 的新鲜度由 `ExecStartPre` fetch 供给，checkout 一致时零网络
  请求。任何**无法证明新鲜**的状态（origin ref 缺失、无 release tag、
  import 源无法定位）都以同样方式失败并带 `reason=` 字段——绝不静默
  放行。
- 落后或无法证明时输出结构化
  `runner_source_stale install=... source=... head=... origin_main=...
  allowed=false fix=...` 行并 fail fast：不取 slot、不领取、不改标签。
  `fix` 是对 deploy home 的精确 editable 强制重装命令。
  `allow_stale_runner: true`（getting-started）把同一行降级为告警，
  供离线/受限网络机器使用——绝不跳过检查。
...
1. **shell 层 `ExecStartPre` preflight**（fetch + `merge --ff-only` +
   CLI probe 自愈）是最外层防线：由 systemd 执行、不 import 任何
   runner 代码，因此**不依赖 runner 版本**。保持已安装 unit 跟上模板
   （`install-units` 与启动前检查的 `unit_drift` 自愈），这道 preflight
   就永远存在。
...
- `pi_retry_429` — provider 限流重试每次一次 WARNING（Issue #321）：
  ...
  等待期间持有 slot（本 run 等待时不认领新 Issue）。连续 5 次 429
  之后走既有失败路径——与非重试退出抛同一类异常（session 前终态
...
- **CLI：**裸 `orbi` tick，以及 `add`、`status`、`doctor`、`setup`、
  `install-units`、`session` 子命令，包括 `--config` 和
  `ORBI_CONFIG` 环境变量。
- **配置：**`orbi.toml` 字段（`source_repos`、`repo_dir`、`deploy_home`、
  `workspace_root`、`prompt`、`prompt_review`、`base_branch`、
  `git_transport`、`active_milestone`、`max_concurrency`、
  `allow_stale_runner`、`model_wait_dead_seconds`、`model_wait_probe_url`、
  `model_wait_probe_seconds`、`skills`、`context_files`、`pi_provider`、
  `pi_model`、`pi_thinking`、`pi_providers`），以及由 systemd
  `EnvironmentFile` 加载的 gitignored `.orbi/env` 文件。
