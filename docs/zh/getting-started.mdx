---
description: "自托管一个会合并和发布的 AI coding agent 需要什么?"
---
# 快速开始

你需要 GitHub 仓库、Orbi CLI 和调度器、已认证的模型 provider，以及运行交付闭环的本地用户会话。
...
为 launchd）；uv 会在需要时提供 Python 3.14。安装器还会替你安装
[Pi](https://github.com/earendil-works/pi) CLI（通过
`npm install -g --ignore-scripts @earendil-works/pi-coding-agent`）——
Node ≥ 22.19（npm 随附其中）是它唯一不替你安装的前提：请从
[官方下载页](https://nodejs.org/en/download)自行安装 Node，安装器会校验版本。
...
`git`、`gh`、`uv` 和调度器 user session——Linux 为 systemd，macOS
为 launchd）；它从不自动安装
系统包、从不安装 `gh`、也不执行 `gh auth login`（你先登录，setup 只
验证登录状态）。`orbi check` 是覆盖同一清单的只读前置条件门禁，另外校验
Python 版本、`pi` CLI、配置文件、各仓库访问权限、git transport 和模型
provider——失败即快速退出，并给出原因、修复动作和官方文档链接。

| 要求 | 用途 | 检查 |
|---|---|---|
| Python 3.14 | Runner 和测试契约（CI 固定同一 minor 版本） | 由 `uv` 提供 |
| [uv](https://docs.astral.sh/uv/) | 把 `orbi` CLI 装进隔离的 tool 环境（setup 的 CLI editable 步骤会调 `uv tool install`） | `uv --version` |
| [Pi](https://github.com/earendil-works/pi) CLI，需 Node ≥ 22.19.0 | 开发 agent；每个任务一个完整 session——由一键安装器安装（经 npm）；手动安装命令：`npm install -g --ignore-scripts @earendil-works/pi-coding-agent` | `pi --version` |
| Git | worktree、分支、base 新鲜度 | `git --version` |
| [GitHub CLI（`gh`）≥ 2.94](https://github.com/cli/cli/blob/trunk/docs/install_linux.md) | Issue、标签、PR、merge | `gh --version`，再 `gh auth status` |
| systemd user session（Linux） | 每 5 分钟触发一个 tick 的用户级 timer | `systemctl --user status` |
| launchd（macOS） | 每 5 分钟触发一个 tick 的用户级 agent——尚未在真机验证，欢迎回报 | `launchctl print gui/$(id -u)` |
...
从 GitHub CLI 官方仓库安装 `gh`——最低已验证版本为 **2.94.0**：Runner 的
队列扫描用到 `gh api --paginate --slurp`（gh 2.48.0 引入）和原生的
`blockedBy` 依赖字段（gh 2.94.0 引入）。过旧的包会让 `orbi setup` 通过，
但每个 tick 都以 `unknown flag: --slurp` /
`Unknown JSON field: "blockedBy"` 失败，永远领不到 Issue——Ubuntu 24.04
的发行版包（2.45.0）正是这种情况；Fedora（2.97.0）与 Arch（2.100.0）
当前原生满足最低版本。在 Ubuntu/Debian 上，用官方 apt 仓库安装或修复
（[官方 Linux 安装说明](https://github.com/cli/cli/blob/trunk/docs/install_linux.md)）：
...
Pi 的 npm 包在 `engines` 里声明了 **Node ≥ 22.19.0** 的下限，但 npm 并不
强制执行：在更旧的 Node 上，上面的 `npm install` 仍然报告成功，随后每一次
`pi` 调用都会因 bundle 里的 `SyntaxError` 崩溃——这是安装路径上最难诊断的
故障。Debian trixie 和 Ubuntu 24.04 的发行版默认 Node 都低于该下限。修复
方法：安装 Node ≥ 22.19，再用上面的命令重装 Pi。`orbi check` 会真实执行
`pi --version`（只在 PATH 上找到并不算通过），探针失败时会在 `fix=` 里
给出这个下限。
...
在 clone 目录里安装，`--python` 传一个**兼容**的解释器（Issue #861）：
系统 Python 是 3.14 或更新（Fedora 43、当前 Arch 自带）时直接用
`python3`，否则传 `3.14` 让 uv 自动供应或选择——Ubuntu 24.04 系统自带
Python 3.12，用精确路径把安装钉死到该系统解释器会因依赖解析失败而退出
1；而 `python3`、`3.14` 这类宽泛写法会让 uv 选取或供应一个满足要求的
解释器（在这类宿主机上即托管安装的 CPython 3.14）：

```bash
# 系统 Python >= 3.14（Fedora 43、当前 Arch）：
uv tool install --force --reinstall --editable --python python3 .
# 更老的系统 Python（如 Ubuntu 24.04 自带 3.12）——uv 自动供应 3.14：
...
在 import 阶段就死了，刷新根本没机会运行）——这类损坏由 service 的第一个
`ExecStartPre` 步骤在 Python 进程外自愈：它探测 `orbi --version`，
探测失败时执行同一条 editable 强制重装——排在第二步（`orbi
sync-engine-source`，Issue #535）之前，因为第二步需要可用的 CLI
（完整机制见[运维](/zh/operations)）。
`orbi doctor` 报告一致性
...
| `worktree_retain_hours` | 否 | `72` | tick 开头任务 worktree 回收的保留窗口（Issue #760，仅宿主/部署侧——仓库的 `.github/orbi.toml` 不可携带）：已关闭超过该小时数的任务 worktree 会被自动移除（OPEN 的票和有活跃 run 的现场绝不触碰）。机制与 `worktree_reclaimed` journal 行：[运维](/zh/operations)。必须是有限正数小时，否则启动 fail fast |
| `workspace_root` | 否 | `..` | 包含任务 worktree 和 agent 可修改仓库的目录 |
| `prompt` | 否 | `prompts/prompt.md` | 实现者 prompt 模板（`{{SOURCE_REPO}}` 等占位符由 Runner 渲染） |
| `prompt_review` | 否 | `prompts/prompt_review.md` | PR 后独立审查 session 的 review prompt 模板 |
| `issue_comments_limit` | 否 | `20` | 受信任评论注入上限（Issue #745，仅宿主/部署侧——仓库的 `.github/orbi.toml` 不可携带）：`{{ISSUE_COMMENTS}}` 占位符注入多少条受信任评论（OWNER/MAINTAINER/MEMBER/COLLABORATOR 或 runner 自己的 App——与恢复现场同一套 `authorAssociation` 信任判据），按时间正序、保留最新。超出上限时丢弃更早的，并在注入内容里标明省略条数——绝不静默截断。模板不含该占位符时不发起任何评论读取（#745 之前的行为）。必须是正整数，否则启动 fail fast |
| `base_branch` | 否 | `main` | 交付 base 分支；每个任务 worktree 都从冻结的 `origin/<base_branch>` SHA 创建 |
| `git_transport` | 否 | `ssh` | 交付 checkout 的传输方式（Issue #580）。`ssh`（默认）：git 数据操作走 `git@github.com:owner/repo.git`，用机器的 SSH key；setup 会把 HTTPS origin 迁移过去。`https`：origin 保持 `https://github.com/<repo>.git`，`git ls-remote`/fetch/push 的凭据走 `gh` credential helper（`gh auth login --with-token`）——无 SSH 私钥的纯 token sandbox 路线。其他值直接让配置加载失败 |
| `engine_source_track` | 否 | `main`（`install.sh` 新安装为 `release`） | engine source 更新通道（Issue #535，host/deploy-only——仓库的 `.github/orbi.toml` 永远不允许携带）：每次 service 启动时 deploy home checkout 跟随的源。缺省/`main`：fast-forward `origin/main`（#535 之前 dogfood 的行为）——开发主线，每次提交都变。`branch:<name>`：fast-forward `origin/<name>`。`release`（别名 `stable`）：最新正式 semver tag（排除 pre-release），detached——跟随最新已发布版本，只在发版时变化。`tag:<name>`/`sha:<40-hex>`：精确锁定，detached——不改值就永不移动，版本变更永远是一次有意操作（要求可复现的部署用它锁定）；回滚就是把值改回去。checkout 脏、tag/SHA 缺失或任何不可验证状态都会以结构化行让启动失败（见[运维](/zh/operations)） |
...
| `auto_next_milestone` | 否 | `true` | 当前 Milestone 关闭且队列为空时是否自动推进。`false` 保持 `active_milestone` 不变，记录待确认告警并创建一个不带 `ai-ready` 或 `p0`、不会被领取的确认 Issue；人工推进后旧通知会自动关闭。非法值立即失败 |
| `max_concurrency` | 否 | `1` | 本机并发 Orbi 任务数和启用的 Runner timer 数（`1`..`5`，即 `MAX_RUNNER_INSTANCES` 声明上限——Issue #827；实际能稳定跑几条取决于 CPU/内存峰值，调高前先实测） |
| `allow_stale_runner` | 否 | `false` | Runner 执行代码新鲜度门禁的离线逃生口（Issue #525）：领取任何 Issue 之前，Runner 必须证明自己执行的代码与配置的 engine source 通道一致——即 `engine_source_track`（Issue #535），缺省 `origin/main`，绝不是交付 `base_branch`（editable 安装看 import 所在 checkout 的 `HEAD`；非 editable 看安装版本与最新 release tag——全部是本地 git 读操作）。落后或无法证明时输出结构化 `runner_source_stale` 行（事实 + 修复命令）并 fail fast。`true` 把同一行降级为告警放行，供离线/受限网络机器继续运行——绝不跳过检查。非法值立即失败 |
| `attribution_footer` | 否 | `true` | 在 Orbi 创建的 PR 正文和 Release notes 末尾追加一行回链；设为 `false` 可关闭，必须是布尔值 |
| `human_review_gate` | 否 | `false` | 人工验收门禁（Issue #763，仅宿主/部署侧——仓库的 `.github/orbi.toml` 不可携带）：`true` 时每个交付在 PR 打开时向 Issue 发一条验收清单（栏一：本次 run 已验证的证据——真实测试结果与覆盖层；栏二：按改动路径生成的机器验不了的最小集合——业务意图、UI 视觉与文案、外部环境、证据缺失）。栏二非空时评审轮次在任何 Pi session 之前短路：票回 `ai-ready`（opened-PR 标签保留作 resume 锚）、槽位释放，此后每个 tick 只花一次 label 读——等待绝不消耗评审轮次预算。人用只有人能打的 `ai-human-review` 标签确认清单，下一 tick 照常评审/合并（`--match-head-commit` 零改动）。栏二为空是目标状态：清单明写无需人工介入，交付不要求标签直接通过。默认关闭：关闭时交付/评审/merge/release 与之前逐字节一致。必须是布尔值，否则配置加载立即失败 |
...
| `release_ci_wait_seconds` | 否 | `1800` | release 门禁 CI 上限（Issue #268）：release 状态机在 tick 内等待 release commit 上 pending（queued/in_progress）检查的最长时间，超时以独立原因失败——它只是 release 机的纯上限保护，绝不是交付等待机制（Issue #788 已移除交付路径的 CI 等待：pending 检查把交付顺延到下一 tick）。等待期间以 `release_waiting_ci` journal 行和进度评论更新可见；最终结论说了算。必须是有限正数，否则启动 fail fast |
...
| `review_pi_provider` | 否 | 未设置 | 评审会话的 provider 覆盖；未设置时回落到 `pi_provider` |
| `review_pi_model` | 否 | 未设置 | 评审会话的模型覆盖；未设置时回落到 `pi_model` |
| `review_pi_thinking` | 否 | 未设置 | 评审会话的思考级别覆盖；未设置时回落到 `pi_thinking` |
| `pi_extensions` | 否 | `[]` | 可重复的 extension 表：`source` 必须是锁定版本的 `npm:`/git 来源或配置文件相对的现有本地路径；`enabled` 默认开启，`env` 只注入当前 Pi 子进程。每次运行都传入 `--no-extensions`，不会发现全局 Pi extension。 |
...
base_branch = "main"
engine_source_track = "release"  # Issue #535：engine source 通道（见运维）
max_concurrency = 1
skills = []
context_files = []
```
...
在本仓库的交付量级下，这是性质之别，不是程度之别：本仓库自身交付的
中位数约为每次交付 220 万 token（implement 与 review
...
| PR 落后于最新 base | Issue `ai-fix-needed`，review 评论写明具体闸门失败原因：落后 base 时带 `origin/<base>` 的 SHA，merge 冲突时带 `mergeable` 的实际值（Issue #879） | 自动：下一个审查 session 在会话内把最新 `origin/<base>` merge 进分支、解冲突、重跑全量测试 |
...
`N` 时启用 `@1` 到 `@N`（上限 `MAX_RUNNER_INSTANCES = 5`；各自触发自己的 service
...
容量为 `N` 时 N 个实例可以同时运行；容量和启用的 timer 数都由
