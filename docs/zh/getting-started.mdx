...
| `engine_source_track` | 否 | `main`（`install.sh` 新安装为 `release`） | engine source 更新通道（Issue #535，host/deploy-only——仓库的 `.github/orbi.toml` 永远不允许携带）：每次 service 启动时 deploy home checkout 跟随的源。缺省/`main`：fast-forward `origin/main`（#535 之前 dogfood 的行为）。`branch:<name>`：fast-forward `origin/<name>`。`release`：最新正式 semver tag（排除 pre-release），detached。`tag:<name>`/`sha:<40-hex>`：精确锁定，detached——回滚就是把值改回去。checkout 脏、tag/SHA 缺失或任何不可验证状态都会以结构化行让启动失败（见[运维](/zh/operations)） |
