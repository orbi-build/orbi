...
`src/muyan_pilot/` package 目录——新增 package 模块**不需要重装**
...
checkout 根目录**没有** `muyan_pilot.py`（与 package 同名的 flat 文件
会遮蔽 checkout 根目录在 sys.path 上的一切进程的已安装 package）；
直接执行入口 `python3 -m muyan_pilot.cli` 运行与 console script 完全
相同的代码，保留为开发/兼容路径。
...
| `repo_dir` | 否 | `.` | service 启动的 Runner checkout（`src/muyan_pilot/` package 所在目录） |
...
`-m` 运行 Runner package 模块——已安装的 `muyan-pilot` console script
运行的是同一个 tick。）
