...
| Pi 起来了但模型一直不响应 | session JSONL 停在 `model_wait`；死寂窗口后 journal 出现 `model_wait_dead` 行和 `run_failed ... reason=model_wait_dead_stale_...` | 模型请求挂死——模型 server/proxy 挂了、连接断了，或进程还活着但请求被挂住（行上 `upstream_alive=true` 是证据，不是豁免）——用一次真实 `pi --print` 调用检查 endpoint，重启后重新派活（Issue 带现场 `ai-blocked`） |
