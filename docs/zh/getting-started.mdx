...
仓库带一份提交的 example；真实配置是本地状态（gitignored）。

`orbi setup` 会在配置缺失时从仓库提供的 example 创建 `orbi.toml`。

配置保存在 `orbi.toml`（相对路径相对配置文件目录解析）。`ORBI_CONFIG`
可选择其他文件，`orbi check` 会校验它。完整参考见[配置参考](/zh/configuration)。
...
外部单仓库部署请将 `deploy_home` 设为 Orbi checkout，并参阅[配置参考](/zh/configuration)。
