...
| `model_wait_dead_seconds` | 否 | `1800` | 模型请求挂死阈值（Issue #228）：`model_wait` 静默（最近一条**完整** session 事件是 tool result 且无新事件）多少秒后 Runner 杀掉 Pi 会话并 fail fast。它度量的是完整 session 事件之间的静默——Pi 不向 JSONL 写 token 级进度，所以不是 token 级模型 timeout，持续追加事件的慢生成永不触发。默认 1800（30 分钟）——慢本地模型（如 ~17 tokens/s 的 Qwen 27B，llama-server 请求超时 1200 秒）可以存活超过 10 分钟的完整消息。必须是有限正数，否则启动 fail fast |
| `model_wait_probe_url` | 否 | 未设置 | 模型吞请求探针（Issue #233）：模型的 `/slots` 端点（如 `http://127.0.0.1:18082/slots`，agent 面向的端口——local-llm-kv-cache 代理从 llama.cpp 透传该端点）。Pi 冻结在 `model_wait` 时 Runner 探测它；当**所有** slot 持续 `model_wait_probe_seconds` 宽限都报告空闲（`is_processing=false`）时，请求被吞（模型进程活着、连接 ESTABLISHED，但没有东西在生成——#231 现场），Pi 会话被快速杀掉，远早于 `model_wait_dead_seconds`。未设置 = 探针禁用（run 只由 `model_wait_dead_seconds` 兜底，#233 之前的行为）。必须是非空 `http://`/`https://` URL，否则启动 fail fast。探针是纯旁路：探测失败视为无证据，永不导致交付失败 |
| `model_wait_probe_seconds` | 否 | `60` | 吞请求探针的持续空闲宽限（Issue #233）：`model_wait` 期间“所有 slot 空闲”状态需持续多少秒才判定请求被吞并杀掉 Pi。相比挂死阈值（30 分钟）很短（60 秒），吞请求约 1 分钟就能恢复而不是约 30 分钟；又足够长，正在被调度进 slot 的请求不会被误判为吞请求。必须是有限正数，否则启动 fail fast |
...
| Pi 起来了但模型一直不响应 | session JSONL 停在 `model_wait`；死寂窗口（`model_wait_dead_seconds`，默认 1800 秒——完整 session 事件之间的静默，不是 token 级进度）后 journal 出现 `model_wait_dead` 行和 `run_failed ... reason=model_wait_dead_stale_...` | 模型请求挂死——模型 server/proxy 挂了、连接断了，或进程还活着但请求被挂住（行上 `upstream_alive=true` 是证据，不是豁免）——用一次真实 `pi --print` 调用检查 endpoint，重启后重新派活（Issue 带现场 `ai-blocked`）；仍在生成的慢本地模型会存活过窗口（只有模型确实需要超过 30 分钟完整事件静默时才调大 `model_wait_dead_seconds`） |
