...
`nemotron-3-ultra-550b-a55b`、`nemotron-3-super-120b`、`minimax-m3`、`minimax-m2.7`、`z-ai/glm-5.2:free`、`google/gemma-4-31b-it`、`cohere/north-mini-code`、`thinkingmachines/inkling`、`poolside/laguna-s-2.1`、`liquid/lfm-2.5-2.6b`。
...
除非已经核实所选模型当前的 Pi/OpenRouter contract，否则不要添加 `thinkingLevelMap`；需要时只使用该 contract 给出的模型专属映射，本文不臆造映射。按任务选择可以优先考虑：`north-mini-code` 用于小型编码/搜索，`nemotron-3-ultra-550b-a55b` 或 `nemotron-3-super-120b` 用于较长推理，`glm-5.2`/`minimax-m3` 用于通用推理，Gemma 或 LFM 用于轻量实验。这些只是实践建议，不是 benchmark 结果；本文**未实测**。
