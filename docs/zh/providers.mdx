...
| GitHub Models | `github-models.json` | `GH_MODELS_TOKEN` | `openai/gpt-4.1-mini` | 免费原型额度；模型和限额动态变化 | [#319](https://github.com/orbi-build/orbi/issues/319) |
...
### GitHub Models：免费原型额度

GitHub Models 为 GitHub 账号提供 OpenAI-compatible endpoint。这是一条与 Orbi 同在 GitHub、无需额外注册的原型路径，但免费额度属于 GitHub Models 服务限制，不是 Orbi 或 Copilot 的额度。
...
[官方 GitHub Models catalog](https://models.github.ai) 是动态的。本指南核对时清单中的模型 ID 为：`openai/gpt-4.1`、`openai/gpt-4.1-mini`、`openai/gpt-4o`、`openai/o4-mini`、`meta/llama-3.3-70b-instruct`、`meta/llama-4-scout-17b-16e-instruct`、`microsoft/phi-4`、`mistral-ai/mistral-small-2503`、`deepseek/DeepSeek-R1` 和 `xai/grok-3-mini`。可用性可能因账号不同，也可能随时变化；提交的模板固定使用 `openai/gpt-4.1-mini` 作为示例。
...
本仓库没有发起真实 GitHub Models 请求，也没有测量耗尽、重置或 HTTP 429 行为。所有额度和模型可用性都应明确视为**未实测**；运行前请查看当前的 [GitHub Models 限制文档](https://docs.github.com/en/github-models/github-models-pricing)。不要从模型清单或一次成功请求推导每日额度。
