...
**官方静态口径（2026-09-04 核对）：** OpenRouter 免费模型文档给出约 **20–1000 次/日**的账号级免费层范围，具体取决于账号和当前政策（[rate limits](https://openrouter.ai/docs/api/reference/limits)）。官方没有为本清单提供持久的逐模型每日数字。因此上面的 18 个模型均标注为：**每日上限：未逐模型公布；本文未实测**——不能把这个范围写成任何单个模型的保证。

每次实测都记录 UTC 日期、模型 ID、成功请求数、第一次 `429` 的时间、响应 body/error code 和是否恢复；额度取决于账号、模型可用性和 OpenRouter 当前政策。绝不要记录 API key 或完整 prompt。
...
上面两个模型是**可选 smoke pair**，不是交付门槛。如果实际运行，成功记录应包含精确模型 ID、UTC 时间、`result=OK` 和 Orbi run/PR 证据；`429` 是额度实测失败，不能当作跑通。本 checkout 没有这类成功运行证据。未核实的数字不能写成每日上限。
