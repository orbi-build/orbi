---
description: "Orbi 可以运行在哪些模型 provider 上?"
---
# Pi Provider 模板

Orbi 可以使用 Pi `models.json` 形状模板所代表的模型 provider 和 OpenAI 兼容端点运行。
...
| DeepSeek | `deepseek.json` | `DEEPSEEK_API_KEY` | `deepseek-flash` | Orbi 自身交付自 2026-09-22 起使用（例如 [#1328](https://github.com/orbi-build/orbi/issues/1328)）；[价格](https://api-docs.deepseek.com/quick_start/pricing) | [#305](https://github.com/orbi-build/orbi/issues/305) |
...
| Cloudflare Workers AI（`cf-workers`） | `cloudflare-workers-ai.json` | `CLOUDFLARE_API_TOKEN` | `@cf/meta/llama-3.1-8b-instruct-fp8` | 托管免费额度；必须在账号中核对额度 | [#318](https://github.com/orbi-build/orbi/issues/318) |
...
### DeepSeek：Orbi 自身交付使用的配置

DeepSeek 的[价格页](https://api-docs.deepseek.com/quick_start/pricing)
（2026-09-26 核对）列出两个 API 模型 id，均为 1M 上下文：
`deepseek-flash`（DeepSeek-V4.1-Flash）和 `deepseek-v4-pro`。仓库模板固定
`deepseek-flash`，采用模型完整的 `contextWindow` 1,000,000，`maxTokens`
16,384；Orbi 自身交付自 **2026-09-22** 起选择该模型（例如来源 Issue
[#1328](https://github.com/orbi-build/orbi/issues/1328)）。

使用完整窗口。当 `contextTokens` 超过 `contextWindow` 减去
`reserveTokens`（默认 16,384）时，Pi 会压缩会话，并且只保留约 20,000
个近期 token。因此 131,072 的窗口会在约 114.7K token 处触发压缩；在
2026-09-22 以来的 77 个交付会话中，有 **32 个（42%）** 带有
`"type":"compaction"` 记录。压缩后的交付会丢失已读取的文件内容和已看到的
测试输出，只能重新读取、重新运行；完整的 1M 窗口避免了这些额外轮次。

缓存输入（cached input）的计费远低于未缓存输入，因此这个默认模型对 Orbi
这类缓存密集的会话成本很低；具体价格以价格页为准，会随时变化。

### xAI：Grok 4.20 Reasoning

**2026-09-09** 已核对 xAI 模型目录。请从模板中选择精确的 reasoning 变体：
...
目录报告该模型的最大 prompt 长度为 1,000,000 tokens。模板将该值作为 `contextWindow`，并保守地将 `maxTokens` 保持为 16,384。xAI 额度和真实请求**未实测**；配置运行前请查看 [xAI 官方模型目录](https://docs.x.ai/docs/models) 和账号状态。
...
**2026-09-08** 使用原始模板和 Pi 0.85.1 的真实请求提供了具体证据：`google/gemma-4-31b-it:free` 两次返回 HTTP **429**（间隔约两分钟），响应含 `temporarily rate-limited upstream` 和 `limit_source: upstream_provider_shared_pool`；`cohere/north-mini-code:free` 返回 HTTP **200**，内容为 `ok`。429 的处理建议是稍后重试、绑定自己的 provider key，或更换 provider 路由/模型。这说明 429 可能是上游共享池的瞬时限流，不能据此断言每日额度已耗尽。自 [#321](https://github.com/orbi-build/orbi/issues/321) 起，瞬时 429 会在 run 内按有界退避自动重试（journal 的 `pi_retry_429` 行——从不静默）；Orbi 不会自动轮换模型——撑过全部重试仍 429 时，保留失败 run 证据并手动选择另一个已配置模型。自动的 `quota_exhausted` 分类属于 [#313](https://github.com/orbi-build/orbi/issues/313)，不由本文或模板实现。
...
1. 在 AI Studio 创建或选择 key，然后写入 Orbi 使用的 gitignored 环境文件：
   ...
   已安装的 systemd unit 会加载 `.orbi/env`。手动 tick 时，先按[上手指南](/zh/getting-started)导出变量。
...
Orbi 从不在 session 中切换 provider；瞬时 429 会在 run 内按有界退避自动重试（Issue #321——journal 的 `pi_retry_429` 行，从不静默），撑过全部重试仍 429 的 run 依然失败。保留失败 run 的证据，或在下一次运行手动选择另一家已配置的 provider：
...
1. 在 z.ai 控制台创建 API key，然后放入本地 gitignored 环境文件：
   ...
   systemd service 会通过 `EnvironmentFile` 加载该文件；手动 tick 不会加载。手动运行前先执行 `set -a; . .orbi/env; set +a`，再运行[上手指南的手动 tick 命令](/zh/getting-started)。
...
额度命中时上游返回 HTTP `429`；Orbi 从不在 session 中途切换 provider。瞬时 429 会在 run 内按有界退避自动重试（[#321](https://github.com/orbi-build/orbi/issues/321)——journal 的 `pi_retry_429` 行，从不静默）；撑过全部重试仍 429 的 run 依然失败：保留 Issue/worktree 证据，并在下一次运行手动选择另一家已配置的 provider。[#313](https://github.com/orbi-build/orbi/issues/313)规划了按 provider 预算和 run 边界自动轮换；在该功能实现前，不要把它当作当前行为。本指南中的真实 Groq 请求、usage 计量和 429 响应**未实测**；没有脱敏证据时不要声称 Groq Issue 已跑通。

模型即使出现在 Groq 的实时目录中，也可能被组织级策略禁用。此时真实请求会返回 HTTP `403` 和 `model_permission_blocked_org`（不是 `429`）：请让组织管理员在 [Groq 组织限额设置](https://console.groq.com/settings/limits) 中启用该模型，或改选另一家已配置的 provider。HTTP `429` 表示账号额度/速率限制已触发；排障路径与组织级 `403` 不同。

完整的 OpenAI-compatible 配置见[上手指南](/zh/getting-started)，provider 文章系列见 [Issue #305](https://github.com/orbi-build/orbi/issues/305)。
