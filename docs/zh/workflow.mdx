...
- **Release task**：带 `ai-release` 标签的 `ai-ready` Issue（Issue
  ...
  9. 关闭 title 与发布 version 精确一致的 Milestone（Issue #214）——
     ...
     issue 列表（绝不静默跳过）。
  10. 成功：`ai-merged`（终态）并关闭 release Issue，附成功评论
     （release URL、tag、commit、证据、文档站 Release notes 证据、
     Milestone 证据）。任何失败：单独进入 `ai-blocked`（release 是
     人工决策点——不自动重试），失败评论带 run marker 和具体原因，
     异常向上传播让 tick fail fast。
