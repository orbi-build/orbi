...
- **Release task**：带 `ai-release` 标签的 `ai-ready` Issue（Issue
  ...
  3. 按领取层、门禁层、范围层执行 Milestone 规则。在领取层
     （`_pick_from_scan`），只要 Milestone 还有其他 open Issue
     （`open_issues > 1`），release 就完全不领取，跳过并记录
     `release_milestone_incomplete`；它保持 `ai-ready`，这是可恢复的等待，
     永远不会变成 `ai-blocked`。Milestone 查询无法评估时安全失败，跳过并记录
     `release_milestone_check_failed`。三个普通扫描用
     `release_not_claimed` 跳过 `ai-release` Issue；只有 release fallback
     扫描使用 `allow_release=True`。

     只有领取层通过后，门禁层的 `release_waiting_deliveries` 才会等待同一
     Milestone 中带有 `ai-in-progress`、`ai-pr-opened` 或 `ai-fix-needed`
     的 Issue；只有 `ai-ready` 的 Issue 不会阻塞 release。这个有界等待使用
     `RELEASE_DELIVERIES_WAIT_SECONDS`；超时就是 delivery wait timeout，
     release 进入 `ai-blocked`。因此排在其他 open ticket 后面的 release
     尚未被领取，不会累积这个超时。随后才执行其他发布前门禁：**release
     所属 Milestone 内**无遗留 open 的 `ai-in-progress` /
     `ai-pr-opened` / `ai-fix-needed` Issue（Issue #671；release Issue
     本身除外；无 Milestone 时仍按全仓扫描）、release commit 的 CI 全绿
