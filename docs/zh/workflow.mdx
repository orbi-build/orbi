...
ai-ready                任务被派发（orbi add，或人工加标签）
  ...
  -> ai-merged          Orbi 自动合并模式：成功终态；PR body 的 Fixes #N
                        原生关闭 Issue
  -> ai-awaiting-merge  协作仓库模式：评审和检查已通过；维护者批准并在
                        Issue 评论指定的精确 head 上合并 PR
```
...
| `ai-awaiting-merge` | 协作仓库模式：评审和检查已通过，但仓库要求人工批准；维护者批准并合并评论指定的 PR head |
...
仓库可能要求人工批准评审，即使 Orbi 的评审和检查都已通过。在这种协作模式
下，Orbi 交付 PR 后停止在 `ai-awaiting-merge`；Issue 评论写出精确 head，
请维护者批准并合并。普通模式仍由 Orbi 自动合并。若没有 approving-review
阻塞，或是其他合并失败，仍进入 `ai-blocked`。
