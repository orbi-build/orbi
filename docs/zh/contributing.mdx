---
description: "如何编写一个 AI agent 能交付的 Issue?"
---
# 贡献

写清一个可观测的 runtime outcome 和验收标准，让 Orbi 能够实现、测试、审查并交付。
...
一个 Issue 是**一个 runtime outcome**，二选一形状：
**修复**（当 X，应该 Y，实际 Z）或
**变更**（当 X，用户应该能够 Y；今天不能，因为 Z）：
一个可观测行为、一小批相关文件、含测试。标题应该能当测试名用。根因或期望
...
3. 交付循环认领该票后，**先评审外部 PR**——与 Runner 自有交付同一套
   独立评审循环（review 发现直接在你的分支上修复，CI 全绿要求不变）。
   评审结论是引擎的**最后一步**：结论（改了什么 / 风险 / 测试覆盖 /
   CI 状态）会发到 triage 票上，票停在 `ai-blocked` 等维护者处理。
   引擎**绝不合并**外部 PR，也绝不为其挑选 milestone——是否接纳、
   排入哪个版本由维护者裁定（Issue #842）。
4. 只有被放弃、过期或不可修的 PR 才会被内部重做取代——取代时 PR
   会话里会向贡献者说明取代关系。被贡献者撤回或被拒绝的外部 PR 会
   自动把 triage 票放回就绪队列。维护者手动合并外部 PR 后，triage 票
   作为已交付关闭。
