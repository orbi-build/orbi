...
| `ORBI_PI_PROVIDER`、`ORBI_PI_MODEL`、`ORBI_PI_BASE_URL`、`ORBI_PI_API_KEY` | 四个模型 provider 值（要么全设，要么全不设——只设一部分会启动失败并点名缺失的变量）。entrypoint 每次启动都从注入变量重新生成 `/orbi/.orbi/env` 并把 `PI_API_KEY` 写进去——改了 `ORBI_PI_API_KEY`，下次 `docker run`/`docker restart` 即生效，无需手动编辑；`/orbi/.orbi/pi-providers.json`（一个 provider、一个 model，`apiKey` 引用 `$PI_API_KEY`，key 本身不落 JSON）和 `orbi.toml` 里的 `pi_provider`、`pi_model`、`pi_providers` 只在首次启动生成，之后绝不覆盖。 |
...
| 命令 | 用途 |
| --- | --- |
| `docker logs -f orbi` | setup 结果（`setup=ok` 或 `setup_failed ...`）+ systemd 控制台 |
