---
description: "How do I get an AI agent to deliver my first GitHub Issue as a merged PR?"
---
# Quickstart: your first Issue in five steps

Run the five-step quickstart to configure Orbi, submit an Issue, and let its runner deliver a reviewed and merged PR.
...
If you would rather not run a machine, Orbi Managed Cloud runs the same delivery loop hosted: https://orbi.build/cloud/?ref=docs-quickstart

## 1. Install Orbi

The installer installs the **Pi** CLI for you, but **Node ≥ 22.19.0** is
the one prerequisite it does not provide — install Node yourself first
(on an older Node a Pi install still reports success, and every `pi`
call then crashes). The full list lives in the
[prerequisites table](/getting-started#prerequisites).
