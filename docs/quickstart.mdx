...
The installer does not provide every prerequisite: the Runner drives the
**Pi** CLI, which needs **Node ≥ 22.19.0** — install Pi yourself with
`npm install -g --ignore-scripts @earendil-works/pi-coding-agent` (on an
older Node the install still reports success, and every `pi` call then
crashes). The full list lives in the
