...
`nemotron-3-ultra-550b-a55b`, `nemotron-3-super-120b`, `minimax-m3`, `minimax-m2.7`, `z-ai/glm-5.2:free`, `google/gemma-4-31b-it`, `cohere/north-mini-code`, `thinkingmachines/inkling`, `poolside/laguna-s-2.1`, and `liquid/lfm-2.5-2.6b`.
...
Do not add `thinkingLevelMap` unless the selected model's current Pi/OpenRouter contract has been verified. If it is needed, use the model-specific mapping documented by that contract; this guide does not invent one. For task selection, prefer `north-mini-code` for small code/search tasks, `nemotron-3-ultra-550b-a55b` or `nemotron-3-super-120b` for long reasoning when available, `glm-5.2`/`minimax-m3` for general reasoning, and Gemma or LFM for lightweight experiments. These are practical suggestions, not benchmark results; **not tested** here.
