...
| GitHub Models | `github-models.json` | `GH_MODELS_TOKEN` | `openai/gpt-4.1-mini` | Free prototyping allowance; model availability and limits are dynamic | [#319](https://github.com/orbi-build/orbi/issues/319) |
...
### GitHub Models: free prototyping allowance

GitHub Models provides an OpenAI-compatible endpoint for GitHub accounts. It is a useful no-extra-registration prototype path for Orbi, but the free allowance is a GitHub Models service limit, not an Orbi or Copilot entitlement.
...
The [official GitHub Models catalog](https://models.github.ai) is dynamic. The following model IDs were listed in the catalog when this guide was checked: `openai/gpt-4.1`, `openai/gpt-4.1-mini`, `openai/gpt-4o`, `openai/o4-mini`, `meta/llama-3.3-70b-instruct`, `meta/llama-4-scout-17b-16e-instruct`, `microsoft/phi-4`, `mistral-ai/mistral-small-2503`, `deepseek/DeepSeek-R1`, and `xai/grok-3-mini`. Availability can differ by account and can change without a repository change. `openai/gpt-4.1-mini` is the example pinned in the committed template.
...
This repository has not made a real GitHub Models request or measured exhaustion, reset, or HTTP 429 behavior. Treat every quota and model-availability claim as **not measured** and check the current [GitHub Models limits documentation](https://docs.github.com/en/github-models/github-models-pricing) before a run. Do not infer a daily allowance from the model catalog or from a successful request.
