...
There is no automatic fallback. Keep the same `orbi.toml` selectors and replace the provider file/key with another provider from the [catalog](#catalog), then validate again. For example, switch to the existing OpenRouter template and its selected model:
