---
description: "Which model providers can Orbi run on?"
---
# Pi provider templates

Orbi can run with the model providers represented by its Pi `models.json`-shaped templates and an OpenAI-compatible endpoint.
...
| DeepSeek | `deepseek.json` | `DEEPSEEK_API_KEY` | `deepseek-flash` | Used for Orbi's own deliveries since 2026-09-22 (e.g. [#1328](https://github.com/orbi-build/orbi/issues/1328)); [pricing](https://api-docs.deepseek.com/quick_start/pricing) | [#305](https://github.com/orbi-build/orbi/issues/305) |
...
| Cloudflare Workers AI (`cf-workers`) | `cloudflare-workers-ai.json` | `CLOUDFLARE_API_TOKEN` | `@cf/meta/llama-3.1-8b-instruct-fp8` | Hosted free allocation; quota must be checked in the account | [#318](https://github.com/orbi-build/orbi/issues/318) |
...
### DeepSeek: the configuration Orbi's own deliveries run on

DeepSeek's [pricing page](https://api-docs.deepseek.com/quick_start/pricing)
(checked 2026-09-26) lists two API model ids with a 1M context,
`deepseek-flash` (DeepSeek-V4.1-Flash) and `deepseek-v4-pro`. The committed
template pins `deepseek-flash` with the model's full `contextWindow`
1,000,000 and `maxTokens` 16,384, and Orbi's own deliveries have selected it
since **2026-09-22** (e.g. source Issue
[#1328](https://github.com/orbi-build/orbi/issues/1328)).

Use the full window. Pi compacts a session once `contextTokens` exceeds
`contextWindow` minus `reserveTokens` (16,384 by default), and keeps only
about 20,000 recent tokens. A 131,072 window therefore compacted at roughly
114.7K tokens, and **32 of the 77 delivery sessions since 2026-09-22 (42%)**
carry a `"type":"compaction"` entry. A compacted delivery loses the files it
read and the test output it saw, so it re-reads and re-runs; the full 1M
window avoids those extra rounds.

Cached input is billed far below uncached input, which is why this default
model stays cheap for Orbi's cache-heavy sessions; the pricing page carries
the current numbers, which change.

### xAI: Grok 4.20 Reasoning

The xAI model catalog was checked on **2026-09-09**. Select the exact reasoning variant from the template:

```toml
pi_providers = ".orbi/pi-providers.json"
pi_provider = "xai"
pi_model = "grok-4.20-0309-reasoning"
```

The catalog reports a 1,000,000-token maximum prompt length for this model. This template uses that value as `contextWindow` and keeps `maxTokens` at 16,384 as a conservative request cap. xAI quota and a real request were **not tested**; consult the [official xAI model catalog](https://docs.x.ai/docs/models) and your account when configuring a run.
...
The template's selected model is `google/gemma-4-31b-it:free`. Select any ID that is actually present in your copied file; the Runner validates the provider, endpoint, API, model list, and `$OPENROUTER_API_KEY` reference before dispatch:
...
On **2026-09-08**, two real requests using the original template and Pi 0.85.1 produced concrete evidence: `google/gemma-4-31b-it:free` returned HTTP **429** twice, about two minutes apart, with `temporarily rate-limited upstream` and `limit_source: upstream_provider_shared_pool`; `cohere/north-mini-code:free` returned HTTP **200** with `ok`. The 429 remedy was to retry later, bind a personal provider key, or switch provider route/model. This shows that a 429 can be transient upstream shared-pool limiting, not proof that the daily allowance is exhausted. A transient 429 is retried in-run with bounded backoff since [#321](https://github.com/orbi-build/orbi/issues/321) (the `pi_retry_429` journal lines — never silent); Orbi does not automatically rotate models — for a 429 that survives every retry, preserve the failed run evidence and manually select another configured model. Automatic quota classification (`quota_exhausted`) belongs to [#313](https://github.com/orbi-build/orbi/issues/313) and is not implemented by this documentation or template.
...
1. Create or select a key in AI Studio, then add it to the gitignored environment file used by Orbi:
   ...
   The installed systemd unit loads `.orbi/env`. For a manual tick, export it first as described in [Getting started](/getting-started#path-b-a-pi_providers-file-orbi-manages-the-selection).
...
Orbi never switches providers mid-session; a transient 429 is retried in-run with bounded backoff (Issue #321 — the `pi_retry_429` journal lines, never silent), and a 429 that survives every retry still fails the run. Preserve the failed run evidence, or manually select another configured provider for the next run:
...
```text
journal: provider=cf-workers model=@cf/meta/llama-3.1-8b-instruct-fp8 run_id=<run-id>
...
A quota hit is returned upstream as HTTP `429`; Orbi never switches provider mid-session. A transient 429 is retried in-run with bounded backoff ([#321](https://github.com/orbi-build/orbi/issues/321) — the `pi_retry_429` journal lines, never silent); a 429 that survives every retry still fails the run: preserve the Issue/worktree evidence, and manually select another configured provider for the next run. The planned per-provider budget and automatic run-boundary rotation is tracked in [#313](https://github.com/orbi-build/orbi/issues/313); it is not yet a promise of current behavior. Real Groq requests, usage measurements, and 429 responses are **not tested** in this guide; do not claim a successful Groq Issue run without recording redacted evidence.

A model can also be present in Groq's live catalog but blocked by an organization policy. In that case a real request returns HTTP `403` with `model_permission_blocked_org` (not `429`): an organization administrator must enable the model at [Groq organization limits](https://console.groq.com/settings/limits), or you must select another configured provider. HTTP `429` means the account quota/rate limit was reached; follow the separate quota troubleshooting path above.

See [Getting started](/getting-started#path-b-a-pi_providers-file-orbi-manages-the-selection) for the existing OpenAI-compatible setup and [Issue #305](https://github.com/orbi-build/orbi/issues/305) for the provider article series.
