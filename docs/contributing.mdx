---
description: "How do I write an Issue that an AI agent can deliver?"
---
# Contributing

Write one observable runtime outcome with clear acceptance criteria so Orbi can implement, test, review, and deliver it.
...
One Issue is **one runtime outcome**, in one of two shapes: a **fix**
(when X, should Y, actually Z) or a **change** (when X, the user should
be able to Y; today they cannot because Z) — one observable behavior, a
handful of related files, tests included. The
...
Write the Issue title in English. The body may include Chinese, but put the
English description and acceptance criteria first so English-speaking readers
can follow the task without translation.
...
4. Run the full contract locally (see
   [Testing](/testing) for the exact commands). How you set up your
   local environment is your own business — it is not a repository
   contract, and the repository maintains no local test chain
   (Issue #569).
...
## External PRs (public repository)

Orbi is a public repository: external Issues and PRs are a **mainline
input**, not an edge case. You keep full freedom over your branch name —
the integration duty is the system's, never the contributor's
(Issue #608).

What happens to an external PR, in order:

1. **CI** runs on every PR, exactly as on the maintainers' own branches.
2. A red CI files one triage Issue per failed job, and that Issue
   **always links the PR**. An external PR (any branch name outside the
   Runner's internal `orbi/<owner>-<repo>-issue-<n>` delivery naming) is
   additionally marked in the Issue body, so neither the Runner nor a
   human reviewer can mistake it for an internal redo.
3. When the delivery loop claims that triage Issue, it **reviews the
   external PR first** — the same independent review loop the Runner
   applies to its own deliveries (review findings are fixed on your
   branch, CI green applies unchanged). The review's outcome is the
   engine's FINAL step: the conclusion (what changed, risks, test
   coverage, CI status) is posted on the triage Issue and the Issue
   waits at `ai-blocked` for a maintainer. The engine NEVER merges an
   external PR and never picks its milestone — accepting the
   contribution and choosing the version is the maintainer's decision
   (Issue #842).
4. Only an abandoned, stale or unfixable PR is superseded by an internal
   redo — and the PR thread then explains the supersession to the
   contributor. A withdrawn or rejected external PR returns the triage
   Issue to the ready queue automatically. When a maintainer merges the
   external PR, the triage Issue closes as delivered.

Review and merge follow the maintainers' pace; no time promises are made
beyond this one: an external PR is never silently redone while it is
open, and the triage trail (Issue with the PR link) is the place where
the status is always visible. Single-repository collaboration is the
current model — fork workflows are not part of the loop.
