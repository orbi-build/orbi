...
checkout=remote=origin branch=main clean=true base_fresh=true remote_url=git@github.com:orbi-build/orbi.git protocol=ssh migrated=false ssh_reachable=true transport_reachable=true
...
changes the exit code. `migrated=true` means setup rewrote an
`origin` on the opposite transport to the configured transport's URL
(a re-run then reports `migrated=false`); `ssh_reachable` is the SSH
`git ls-remote` probe result (`-` in https mode — no SSH probe runs
there) and `transport_reachable` is the probe result of the ACTIVE
transport (Issue #580).
