...
The published image is available for both supported Linux architectures:

```bash
docker pull ghcr.io/orbi-build/orbi:latest
```

Docker Hub is an alternate registry:

```bash
docker pull docker.io/orbibuild/orbi:latest
```

To build from source instead, keep using the repository checkout:
