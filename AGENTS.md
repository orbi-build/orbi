...
- An existing HTTPS remote is never rewritten silently: only the human-run `orbi setup` migrates it with `git remote set-url origin git@github.com:owner/repo.git`; every other path fails fast with the exact migration command. `orbi doctor` reports the transport read-only.
