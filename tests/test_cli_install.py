...
def test_refresh_failure_logs_the_structured_line_with_the_fix_command(
    ...
    with caplog.at_level("ERROR"):
        with pytest.raises(
            runner.CliInstallError, match="the tool env is broken",
        ):
            runner.refresh_cli_install(tmp_path, run_command=boom)
    line = next(
...
def test_two_concurrent_starts_run_exactly_one_install(tmp_path):
    ...
    def start():
        try:
            results.append(
                runner.refresh_cli_install(tmp_path, run_command=run),
            )
...
def test_reinstall_argv_is_the_verified_editable_force_reinstall(tmp_path):
    ...
    _write_pyproject(tmp_path, '[project]\nname = "a"\n')
    calls = []
    runner.refresh_cli_install(tmp_path, run_command=_recorder(calls))
    assert len(calls) == 1
    command, kwargs = calls[0]
    assert command == [
